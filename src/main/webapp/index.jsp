		...
		<div class="container">
			...
			<div class="mapBox">
				<div class="mapBox">
					...
					%>
					<script>
<%-- 					locations = [...locations, ['<div><h3>'+'<%=searchInfo.getNAME()%>'+'</h3><p><%=searchInfo.getRATING()%> 성</p><p>주소: <%=searchInfo.getADDRESS()%></p><p>전화번호: <%=searchInfo.getTEL()%></p><p><button class="btn btn-secondary" onclick= "scrolltoId()" type="submit">자세히 알아보기</button></p></div>', <%=searchInfo.getX()%>, <%=searchInfo.getY()%>]]; --%>
					locations = [...locations, ['<div><h3>'+'<%=searchInfo.getNAME()%>'+'</h3><p><%=searchInfo.getRATING()%> 성</p><p>주소: <%=searchInfo.getADDRESS()%></p><p>전화번호: <%=searchInfo.getTEL()%></p><p><button class="btn btn-secondary" type="submit">자세히 알아보기</button></p></div>', <%=searchInfo.getX()%>, <%=searchInfo.getY()%>]];
